...
- ContentArea：主内容区，承载 Agent 聊天、输入发送、右侧面板、仓库管理、模型选择、代理与 API Key 管理等。**更新**：集成了完整的在线模型系统，包括模型选择、凭证获取和 sidecar 生命周期管理，以及 Portal 环境调试功能。
- 设置页、插件市场页、待办页：独立视图，通过 App 的视图状态切换进入。
- 自定义 Hook：useWorkspaces（工作区/智能体/消息持久化与更新）、useAgentContext（智能体消息监听与查询控制）、useAgent（与 Sidecar 通信）、useTheme（主题切换与系统跟随）、useOnlineModels（在线模型管理与环境切换监听）、useAuth（认证管理）、useI18n（国际化键解析）。
...
## 架构总览
应用采用"Provider 组合 + 自定义 Hook"的状态与行为管理策略，结合 Tauri 命令与事件实现与 Sidecar 的双向通信。主题与国际化通过 Context 提供，贯穿整个组件树。**更新**：新增在线模型系统，通过 useOnlineModels 和 useAuth 实现模型选择、凭证获取和认证管理，同时引入 Portal 环境调试系统，支持生产与本地环境的动态切换。
...
ADVANCED["settings/AdvancedPanel.tsx"]
END
APP --> THEME
APP --> I18N
APP --> AUTH
...
- [src/components/settings/AdvancedPanel.tsx:1-146](file://src/components/settings/AdvancedPanel.tsx#L1-L146)
...
- useOnlineModels：**新增** 管理在线模型列表和 AI 转发 Key，支持环境切换监听、缓存管理和登录状态集成。
...
#### useOnlineModels **新增**
- 目标：管理在线模型列表和 AI 转发 Key，支持环境切换监听。
- 关键能力：
  ...
  - **新增** 环境切换监听：监听 PORTAL_ENV_CHANGE_EVENT 事件，自动清除缓存并重新拉取。
...
Event["PORTAL_ENV_CHANGE_EVENT 事件"] --> ClearCache["清除缓存的 AI 转发 Key"]
ClearCache --> ClearModels["清除模型缓存"]
ClearModels --> ForceRefresh["强制重新拉取模型"]
ForceRefresh --> ReturnModels
```
...
- [src/hooks/useOnlineModels.ts:123-133](file://src/hooks/useOnlineModels.ts#L123-L133)
...
- **更新**：集成在线模型系统，支持线上模型选择、登录引导和凭证获取，以及 Portal 环境调试功能。
...
#### Portal 环境调试系统 **新增**
- 环境切换监听：useOnlineModels 监听 PORTAL_ENV_CHANGE_EVENT 事件，自动清除缓存并重新拉取模型列表。
- 环境切换机制：通过 AdvancedPanel 中的 Toggle 开关切换生产环境（prod）和本地环境（local）。
- 缓存管理：两套环境的 AI 转发 Key 不通用，必须清除重取，确保环境隔离。
- 实时反馈：切换后立即更新当前生效的 origin 显示，符合 baseurl 来源可追溯性要求。

```mermaid
sequenceDiagram
participant User as "用户"
participant Advanced as "AdvancedPanel"
participant Portal as "portalClient"
participant Online as "useOnlineModels"
User->>Advanced : 切换 Portal 环境
Advanced->>Portal : setPortalEnv(mode)
Portal->>Portal : localStorage.setItem(PORTAL_ENV_KEY, mode)
Portal->>window : dispatchEvent(PORTAL_ENV_CHANGE_EVENT)
window->>Online : 触发 PORTAL_ENV_CHANGE_EVENT 监听器
Online->>Online : clearAiForwardingKey()
Online->>Online : clearModelsCache()
Online->>Online : refreshModels(true)
Online-->>User : 更新模型列表
```

图表来源
- [src/components/settings/AdvancedPanel.tsx:28-32](file://src/components/settings/AdvancedPanel.tsx#L28-L32)
- [src/utils/portalClient.ts:46-54](file://src/utils/portalClient.ts#L46-L54)
- [src/hooks/useOnlineModels.ts:123-133](file://src/hooks/useOnlineModels.ts#L123-L133)

章节来源
- [src/components/settings/AdvancedPanel.tsx:19-32](file://src/components/settings/AdvancedPanel.tsx#L19-L32)
- [src/utils/portalClient.ts:30-59](file://src/utils/portalClient.ts#L30-L59)
- [src/hooks/useOnlineModels.ts:123-133](file://src/hooks/useOnlineModels.ts#L123-L133)
...
- **更新**：在线模型系统依赖 portalClient.ts 提供的 API 调用，useAuth.tsx 提供认证令牌，useOnlineModels.ts 管理模型状态，AdvancedPanel.tsx 提供环境切换界面。
...
CT --> ONLINE_MODELS
CT --> AUTH
ADVANCED["AdvancedPanel.tsx"] --> PORTAL
ONLINE_MODELS --> ADVANCED
```
...
- [src/i18n/locales/en.ts:1-704](file://src/i18n/locales/en.ts#L1-L704)
- [src/i18n/locales/zh.ts:1-826](file://src/i18n/locales/zh.ts#L1-L826)
- [src/hooks/useOnlineModels.ts:14-21](file://src/hooks/useOnlineModels.ts#L14-L21)
...
- [src/components/settings/AdvancedPanel.tsx:8](file://src/components/settings/AdvancedPanel.tsx#L8)
...
- **更新**：Portal 环境切换失效：检查 PORTAL_ENV_CHANGE_EVENT 事件是否正确派发，确认 useOnlineModels 的监听器是否正常工作。
...
## 结论
RabbitCoding 前端以 Provider + 自定义 Hook 为核心，结合 Tauri 事件与命令，实现了稳定、可扩展的智能体交互体验。通过完善的主题与国际化、健壮的状态持久化与兜底策略、以及清晰的组件边界，整体架构具备良好的可维护性与扩展性。**更新**：新增的在线模型系统和 Portal 环境调试系统进一步增强了应用的灵活性，通过认证管理、模型选择、凭证获取和环境切换的完整链路，为用户和开发者提供了更便捷的 AI 服务接入方式和调试体验。
...
- 最佳实践
  ...
  - **更新**：Portal 环境调试系统通过事件驱动实现，确保环境切换的实时性和可靠性。
  - **更新**：环境切换监听器自动清理缓存，避免跨环境数据污染。