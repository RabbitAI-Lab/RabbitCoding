...
- 自定义 Hook 聚焦于状态管理与跨组件共享：useWorkspaces、useAgentContext、useTheme、useAgent、useI18n 等。
...
- ContentArea：主内容区，承载 Agent 聊天、输入发送、右侧面板、仓库管理、模型选择、代理与 API Key 管理等。
- 设置页、插件市场页、待办页：独立视图，通过 App 的视图状态切换进入。
- 自定义 Hook：useWorkspaces（工作区/智能体/消息持久化与更新）、useAgentContext（智能体消息监听与查询控制）、useAgent（与 Sidecar 通信）、useTheme（主题切换与系统跟随）、useI18n（国际化键解析）。

章节来源
- [src/App.tsx:29-99](file://src/App.tsx#L29-L99)
- [src/components/sidebar/Sidebar.tsx:17-44](file://src/components/sidebar/Sidebar.tsx#L17-L44)
- [src/components/ContentArea.tsx:30-667](file://src/components/ContentArea.tsx#L30-L667)

## 架构总览
应用采用“Provider 组合 + 自定义 Hook”的状态与行为管理策略，结合 Tauri 命令与事件实现与 Sidecar 的双向通信。主题与国际化通过 Context 提供，贯穿整个组件树。
...
- [src/types/index.ts:1-716](file://src/types/index.ts#L1-L716)
...
- [src/i18n/locales/zh.ts:1-704](file://src/i18n/locales/zh.ts#L1-L704)
...
## 结论
RabbitCoding 前端以 Provider + 自定义 Hook 为核心，结合 Tauri 事件与命令，实现了稳定、可扩展的智能体交互体验。通过完善的主题与国际化、健壮的状态持久化与兜底策略、以及清晰的组件边界，整体架构具备良好的可维护性与扩展性。
