...
- 更新器系统：集成 @tauri-apps/plugin-updater，提供自动更新功能与签名验证。
...
F --> G["打包产物<br/>各平台安装包"]
E --> H["更新器系统<br/>@tauri-apps/plugin-updater"]
H --> I["签名验证<br/>pubkey 字段"]
```
...
- **更新器系统**：集成 @tauri-apps/plugin-updater，提供自动更新功能，支持签名验证与多通道更新。
...
- 关键步骤：
  ...
  - **注入版本号与更新通道**：夜间版将更新器端点指向 nightly 清单。
...
- **更新器配置**：启用更新器插件，配置更新端点、对话框显示、公钥验证。
- 能力集：default.json 定义窗口拖拽、通知、文件系统读写范围、PTY、窗口状态等权限。

**章节来源**
- [src-tauri/tauri.conf.json:1-76](file://src-tauri/tauri.conf.json#L1-L76)
- [src-tauri/capabilities/default.json:1-44](file://src-tauri/capabilities/default.json#L1-L44)

### Rust 依赖与构建（Cargo.toml 与 build.rs）
- 依赖：Tauri 核心、Shell/Fs/Dialog/Notification/PTY、SQLite、reqwest、图像处理、深链等。
- **更新器依赖**：tauri-plugin-updater = "2"。
...
### 更新器系统与签名验证
- **更新器插件集成**：在 src-tauri/src/lib.rs 中启用 tauri_plugin_updater::Builder::new().build()。
- **前端依赖**：package.json 中包含 "@tauri-apps/plugin-updater": "^2"。
- **更新通道配置**：tauri.conf.json 中配置了更新器端点、对话框显示和公钥验证。
- **CI/CD 集成**：GitHub Actions 使用 TAURI_SIGNING_PRIVATE_KEY 和 TAURI_SIGNING_PRIVATE_KEY_PASSWORD 进行更新清单签名。
- **动态端点配置**：夜间版构建时通过 Node.js 脚本动态修改更新器端点指向 nightly 清单。

**章节来源**
- [src-tauri/src/lib.rs:665](file://src-tauri/src/lib.rs#L665)
- [package.json:26](file://package.json#L26)
- [src-tauri/tauri.conf.json:66-73](file://src-tauri/tauri.conf.json#L66-L73)
- [.github/workflows/build.yml:178-180](file://.github/workflows/build.yml#L178-L180)
...
- **更新清单生成**：tauri-action 自动生成 latest.json（供 upgrader 使用），nightly 模式下指向 nightly 更新端点。
- **签名验证**：更新清单使用 TAURI_SIGNING_PRIVATE_KEY 进行签名，客户端通过公钥验证更新包完整性。
...
- Tauri 插件：Shell、Fs、Dialog、Notification、PTY、Window-State、Deep-Link、**Updater** 等。
- 资源依赖：前端 dist、sidecar-bundle.js、原生二进制、Node.js 运行时。
- **更新器依赖**：@tauri-apps/plugin-updater 前端插件与 tauri-plugin-updater Rust 插件。
...
Pkg --> UpdaterFrontend["@tauri-apps/plugin-updater"]
```
...
- **更新器安全性**：通过公钥验证确保更新包完整性，防止中间人攻击。
- **更新通道隔离**：正式版与夜间版使用不同的更新端点，避免测试版本影响生产用户。
...
- **更新器签名验证失败**
  - 症状：更新包下载后验证失败，提示签名不匹配。
  - 排查：确认 TAURI_SIGNING_PRIVATE_KEY 和 TAURI_SIGNING_PRIVATE_KEY_PASSWORD 配置正确，公钥在 tauri.conf.json 中配置一致。
- **更新器权限问题**
  - 症状：应用无法访问更新器功能。
  - 排查：确认 capabilities/default.json 中包含 "updater:default" 权限。
...
## 结论
本项目通过 GitHub Actions 实现了跨平台的自动化打包与发布，结合 Sidecar 资源注入与 Tauri 的能力集与权限模型，提供了稳定的桌面端体验。**新增的更新器系统**进一步增强了应用的可维护性，通过签名验证确保更新的安全性，版本管理策略清晰区分正式版与夜间版，更新通道与签名机制完善，适合在多平台上进行持续交付与快速迭代。
...
- **更新器配置**
  - 确认 package.json 中 @tauri-apps/plugin-updater 版本正确。
  - 验证 src-tauri/tauri.conf.json 中更新器配置。
  - 测试更新器功能：检查更新、下载、安装流程。
...
- **更新器平台兼容性**
  - 所有平台均支持更新器功能，但签名验证机制在不同平台上的实现可能略有差异。
  - 确保更新器端点在各平台均可访问，网络防火墙不会阻断更新请求。
...
### 更新器系统配置详解
- **前端集成**
  - 在 package.json 中添加 "@tauri-apps/plugin-updater": "^2" 依赖。
  - 在应用初始化时导入并使用更新器 API。
- **后端集成**
  - 在 src-tauri/src/lib.rs 中启用 tauri_plugin_updater::Builder::new().build()。
  - 配置更新器权限在 capabilities/default.json 中。
- **配置参数**
  - active: true 启用更新器功能
  - endpoints: 更新清单端点数组
  - dialog: true 显示更新对话框
  - pubkey: 公钥字符串，用于验证更新清单签名
- **CI/CD 集成**
  - 使用 TAURI_SIGNING_PRIVATE_KEY 和 TAURI_SIGNING_PRIVATE_KEY_PASSWORD 进行签名。
  - 夜间版动态配置更新端点指向 nightly 清单。
